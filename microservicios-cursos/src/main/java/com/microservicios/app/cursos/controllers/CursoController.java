...
public class CursoController extends CommonController<Curso, CursoService>{
	...
	@Value("${config.balanleador.test}")
	private String balanceadorTest;
	
	@DeleteMapping("/eliminar-alumno/{id}")
	public ResponseEntity<?> eliminarCursoAlumnoPorId(@PathVariable Long id){
		service.eliminarCursoAlumndoPorId(id);
	...
	public ResponseEntity<?> listar(Pageable  page){
		Page<Curso> cursos = service.findAll(page).map(c -> {
			c.getCursoAlumnos().forEach(ca -> {
				Alumno alumno = new Alumno();
				alumno.setId(ca.getAlumnoId());
				c.addAlumno(alumno);
	...
	public ResponseEntity<?> examenesAlumnos(@RequestBody List<Examen> examenes, @PathVariable Long id){
		Optional<Curso> o = this.service.findById(id);
		if(!o.isPresent()) {
			return ResponseEntity.notFound().build();
		}
